...
This worker report is the Execution Record for the work order's Loop Contract. It should give the coordinator enough evidence to make an Acceptance Decision without relying on chat history. See `docs/loop-contract-model.md` in the AutoLoop repository for the model map.
