...
function Get-WorkerReportIssues {
    ...
    # Keep this logic aligned with check-report.ps1 -Strict. The aggregate
    # checker applies it only to worker reports so coordinator reports stay read-only observations.
    $requiredSections = @(
        "Changed Scope",
        "Verification",
        "Contract Impact",
    ...
    foreach ($sectionName in $requiredSections) {
        ...
        if (-not (Test-SectionHasContent -SectionLines $sectionLines)) {
            $issues.Add("Empty or placeholder-only section: $sectionName")
        }
    }
